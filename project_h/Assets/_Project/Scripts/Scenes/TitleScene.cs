...
public class TitleScene : BaseScene
{
    ...
    private void OnSizeDownloaded(long size)
    {
        ...
        else
        {
            ...
            Managers.UI.ShowPopupUI<UI_MessagePopup>().SetupMessageConfrim($"Wifi 환경이 아니라면 데이터가 많이 소모될 수 있습니다. 다운로드 하시겠습니까? <color=green>({$"{totalSizeUnit}{sizeUnit})</color>"}", showCancelButton: false, callback: (result) =>
            {
                if (result)
                {
                    // Confirm
